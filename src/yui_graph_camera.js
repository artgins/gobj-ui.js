...
/*  The glyph size is the TOOLBAR's, not the graph's: it is shared with
 *  the JSON viewer and the treedb topics row, which are not graphs.  */
...
function camera_button(gobj, icon, event_name, label_key, wide, extra_style)
{
    return ['button', {class: `button ${event_name}`, type: 'button',
        ...
        yui_toolbar_icon(icon, extra_style),
        {
            click: (evt) => {
                evt.stopPropagation();
