    ...
    return [
        ...
        ['button', {class: 'GRAPH_NODE_LABELS button', type: 'button',
            ...
            yui_toolbar_icon('yi-font'),
            {
                click: (evt) => {
                    evt.stopPropagation();
                    gobj_send_event(gobj, "EV_TOGGLE_NODE_LABELS", {}, gobj);
                }
            }
        ],
    ];
...
/************************************************************
 *  The view of the nodes and the labels toggle of the toolbar:
 *  the attr changes, persists, shows on the control, and reaches
 *  the G6 child, which owns the nodes.
 ************************************************************/
function ac_set_node_mode(gobj, event, kw, src)
{
    let priv = gobj.priv;
    let mode = (kw && NODE_MODES.includes(kw.node_mode))? kw.node_mode : "expanded";
