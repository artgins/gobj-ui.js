...
**Where the transport edge comes from** (7.25.5). Until 7.25.4 only from the
host, as `EV_TRANSPORT_STATE` — and two hosts (wattyzer, yunovatios) never
forwarded it, so a websocket drop during a Save left Save and Cancel busy for
ever. The view now also hears it from its SHELL: `yui_shell_set_connection_state()`
— which every single-backend app already calls for the toolbar dot — publishes
`EV_CONNECTION_STATE {connected}` on each edge, and `C_YUI_TREEDB_TOPICS`
subscribes to it in `mt_start`. Nothing to add in such a host. A host with
one transport per view (gui_treedb's `C_TREEDB_VIEW`, gui_agent's
`C_AGENT_TREEDB_VIEW`) keeps forwarding `EV_TRANSPORT_STATE`; both paths can
fire for one edge, and the view answers each write once. The view never
subscribes to the transport itself: a subscription to a `C_IEVENT_CLI` is
forwarded to the backend.

`EV_CONNECTION_STATE` reaches only the subscribers that DECLARE it: an app
that subscribed to every event of its shell with the `subscriber` attr does
not get it (the shell's `mt_publication_pre_filter`), so adding it broke no
app.

```js
// The app, from its transport handlers -- as it already did for the dot:
yui_shell_set_connection_state(shell, false);     // EV_ON_CLOSE
// -> every C_YUI_TREEDB_TOPICS under that shell answers its forms'
//    writes in flight EV_WRITE_REFUSED, and they come back on what was typed.
```

A write is known by its topic AND its serial (`<topic>^<form_write>`): each
...
`instance_keys_of()` (`treedb_write_plan.js`) keeps it from the record the
form was opened on.

**Known limit — the change of hour** (`form_time_value.js`). A
`datetime-local` holds a local wall time and no offset. The night the clocks
go back, one hour happens twice: an epoch in its second pass is shown with the
wall time of the first and read back one hour earlier, so saving that record —
any field of it — moves a WRITABLE time column holding it by -3600 s. The
hour skipped in spring holds no epoch, so nothing read there comes back wrong.
Read-only time columns never go back, and a time pkey2 goes back as it was.
...
to delete the row that sat there when the person answered. Rows (table) or
cards (`C_G6_NODES_TREE`) that another writer deleted while the question was
open are not deleted and the person is told (7.25.4); the rest go on.

Consumer i18n keys of the form's writes and deletes — define them in the app,
or they render as the key itself in every language:

| key | shown when |
|---|---|
| `no session` | a Save asked with no backend session: the write is not sent (7.25.2) |
| `some records were gone before the delete` | a confirmed delete named rows or cards that no longer exist (7.25.4). The text must not promise a refresh: nothing is reloaded, and in the graph there is no table |

```js
// en.js / es.js of the app
"no session": "No session",
"some records were gone before the delete":
    "Some records no longer existed when the delete was confirmed; they were not deleted",
```
...
**`EV_DRAFTS` REPLACES what the host said before** (7.25.5). It only added,
and a topic the host had named before a Save stayed a draft after the Save
that published it ("unsaved schema changes" again). The editor keeps the
host's marks apart from its session's writes (`host_draft_ids()`) and swaps
them whole on each `EV_DRAFTS`, so a host sends the COMPLETE set every time —
an empty `{drafts: {}}` included, which is how "nothing is a draft any more"
is said. What `draft_changed` measures is the node's: the draft against the
SAVED schema on a node newer than yunetas 7.25.3, against the file in use on
a 7.25.3 node (where a topic saved and not yet applied is still a draft until
the Apply).
